...
open_github_ssh_page() {
    ...
    declare -r GITHUB_SSH_URL="https://github.nclmiami.ncl.com/settings/keys"
...
set_github_ssh_key() {
    ...
    local sshKeyFileName="$HOME/.ssh/github_ncl"
    ...
    if [ -f "$sshKeyFileName" ]; then
        sshKeyFileName="$(mktemp -u "$HOME/.ssh/github_ncl_XXXXX")"
    fi
...
test_ssh_connection() {
    ...
    while true; do
        ...
        ssh -T git@github.nclmiami.ncl.com &> /dev/null
        [ $? -eq 1 ] && break
...
main() {
    ...
    print_in_purple "\n • Set up NCL Enterprise GitHub SSH keys\n\n"
